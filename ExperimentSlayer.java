...
@ScriptManifest(authors = { "Pervy Shuya" }, category = "Combat", name = "ExperimentSlayer Pro", version = 5.5, description = "<style type='text/css'>body {background:url('http://img339.imageshack.us/img339/6561/11tf6.gif') no-repeat}</style><html><head><center><head></td><td><center>"
	+ "<html><head>"
	+ "</head><body>"
	+ "<center>"
	...
	private RSTile[] bankToStatue = pathGen(new RSTile(3578, 3526));
	private RSTile[] goingToBank = reversePath(bankToStatue);
	RSTile[] undergroundToStairs = pathGen(new RSTile(3486, 9938));
	RSTile[] Spot1 = pathGen(new RSTile(3558, 9945));
	RSTile[] Spot2 = pathGen(new RSTile(3484, 9939));
	...
	private final int KILLNPCS = 0, BANKING = 1;
	...
	Bones = 526, /* Bones */itpIDs = 526, count = 0, Lifepoints,
	...
	private boolean checkVersion() {
		try {
			...
			log.severe("Error loading version data.");
		}
		...
		if (getVersion() >= latestVersion) {
			...
			if (getVersion() < latestVersion) {
				...
				try {
					...
					log.warning("Problem getting version :/");
					return false;
				}
			}
		}
	...
	private double getCurrentLifepoint() {
	...
	private void checkForFood() {
		if (getCurrentLifepoint() / 10 <= eatAtHp) {
			log("Food time");
	...
	private int getIndexOfClosestRSTile(RSTile[] genPath) {
		int lastDis = 500;
		int index = 0;
		for (int i = 0; i < genPath.length; i++) {
			int dis = distanceTo(genPath[i]);
			if (dis < lastDis) {
				lastDis = dis;
				index = i;
			}
	...
	private RSTile[] editPath(RSTile[] genPath, int index) {
		...
		return temp.toArray(new RSTile[temp.size()]);
	}

	private RSTile[] pathGen(RSTile t) {
		return editPath(cleanPath(generateFixedPath(t)),
				getIndexOfClosestRSTile(cleanPath(generateFixedPath(t))));
	}
			...
			if (distanceTo(centerofpath) > 20) {
				if (distanceTo(statueTile) < distanceTo(ladderTile)) {
					if (!tileOnScreen(statueTile) && distanceTo(statueTile) > 4) {
						walkPathMM(randomizePath(bankToStatue, 2, 2),
								13 + random(2, 4));
	...
	public int loop() {
		if (isLoggedIn()
				&& (System.currentTimeMillis() - lastCheck) >= randomTime) { // Credits
			// to
			// zenzie
			// for
			// his
		...
		if (getCurrentLifepoint() / 10 <= eatAtHp) {
			...
			} else if (getCurrentLifepoint() / 10 <= eatAtHp && getBones) {
				if (getInventoryCount(foodID) >= 1) {
					Status = "Eating food";
					clickInventoryItem(foodID, "Eat");
					return (random(450, 650));
					...
					log.warning("We are out of food! logging out");
					wait(8000);
					stopScript();
				}
			}
		...
		if (getCurrentLifepoint() / 10 <= 5) {
			if (getInventoryCount(teleTabIDs) >= 1) {
				Status = "Teleporting";
				clickInventoryItem(teleTabIDs, "Break");
				log.warning("Emergency teleporting.....Now logging out");
				wait(5000);
				stopScript();
			}
		}

		if (attackStyle == 3 && inventoryContains(avaBag)) {
		...
		case KILLNPCS:
			...
			if (npcMonster != null) {
				...
				if (getMyPlayer().getInteracting() != null) {
					if (getInteractingNPC().getHPPercent() < 1) {
						checkExperimentKC();
				...
				if (getMyPlayer().getInteracting() != null && nextNPC != null
						&& pointOnScreen(nextNPC)
						&& getCurrentLifepoint() / 10 > eatAtHp) {
					Status = "Hovering Experiments";
					moveMouse(nextNPC);
				} else {
		...
		case BANKING:
			if (distanceTo(ladderTile) < distanceTo(bankTile)) {
				...
				if (!tileOnScreen(ladderTile)) {
					walkPathMM(randomizePath(undergroundToStairs, 2, 2),
							13 + random(2, 4));
			...
			if (distanceTo(bankTile) < distanceTo(ladderTile)) {
				if (!tileOnScreen(bankTile) || bankBooth != null
					...
					walkPathMM(randomizePath(goingToBank, 2, 2), 13 + random(2,
							4));
					Location = "Retrieving location";
	...
	class GarrettsPaint {
		...
		void paint(final Graphics g) {
			...
			case 1: // INFO
				...
				drawStringMain(g, "Scripted By: ", getAuthor(), r, 20, 35, 0,
						false);
				drawStringMain(g, "Playing as: ", getMyPlayer().getName(), r,
						20, 35, 2, true);
				drawStringMain(g, "Player Combat Level: ", Integer
						.toString(getMyPlayer().getCombatLevel()), r, 20, 35,
						2, false);
				drawStringMain(g, "Player Health: ", RSInterface.getInterface(
						748).getChild(8).getText().toString()
						+ " Lifepoints", r, 20, 35, 3, true);
				drawStringMain(g, "Player Location: ", getMyPlayer()
						.getLocation().toString(), r, 20, 35, 3, false);
				break;
			case 2: // STATS
				drawPaint(g, r4c);
				drawStats(g);
	...
	public class ExperimentSlayerGUI extends JFrame {
		...
		private void startButtonActionPerformed(ActionEvent e) {
			...
			if (!textField2.getText().isEmpty()) {
				...
				log.severe("Invalid MouseSpeed");
				log("Please leave the value in this box default");
				log("If you do not know what to put");
			}
			...
			if (foodcomboBox3.getSelectedItem().equals("None")) {
				...
				foodName = "Cake";
				Status = "Eating Cake";
			} else if (foodcomboBox3.getSelectedItem().equals("Chocolate cake")) {
				Food = 1897;
				foodName = "Chocolate cake";
				Status = "Eating Chocolate cake";
			} else if (foodcomboBox3.getSelectedItem().equals("Plain pizza")) {
				Food = 2289;
				foodName = "Plain pizza";
				Status = "Eating Plain pizza";
			} else if (foodcomboBox3.getSelectedItem()
					.equals("Pineapple pizza")) {
				Food = 2301;
				foodName = "Pineapple pizza";
				Status = "Eating Pineapple pizza";
			} else if (foodcomboBox3.getSelectedItem().equals("Meat pizza")) {
				Food = 2293;
				foodName = "Meat pizza";
				Status = "Eating Meat pizza";
			} else if (foodcomboBox3.getSelectedItem().equals("Lobster")) {
				Food = 379;
				foodName = "Lobster";
			...
			} else if (foodcomboBox3.getSelectedItem().equals("Monkfish")) {
				Food = 7946;
				foodName = "Monkfish";
				Status = "Eating Monkfish";
			} else if (foodcomboBox3.getSelectedItem().equals("Manta ray")) {
				Food = 391;
				foodName = "Manta ray";
				Status = "Eating Manta ray";
			} else if (foodcomboBox3.getSelectedItem().equals("Sea turtle")) {
				Food = 397;
				foodName = "Sea turtle";
				Status = "Eating Sea turtle";
			} else if (foodcomboBox3.getSelectedItem().equals("Swordfish")) {
				Food = 373;
				foodName = "Swordfish";
				Status = "Eating Swordfish";
			} else if (foodcomboBox3.getSelectedItem().equals("Rocktail")) {
				Food = 15272;
				foodName = "Rocktail";
			...
			if (arrowcomboBox2.getSelectedItem().equals("Bronze arrows")) {
				arrowID = bronzeArrow;
				arrowName = "Bronze arrow";
			} else if (arrowcomboBox2.getSelectedItem().equals("Iron arrows")) {
				arrowID = ironArrow;
				arrowName = "Iron arrow";
			} else if (arrowcomboBox2.getSelectedItem().equals("Steel Arrow")) {
				arrowID = steelArrow;
				arrowName = "Steel arrow";
			} else if (arrowcomboBox2.getSelectedItem().equals("Mithril Arrow")) {
				arrowID = mithrilArrow;
				arrowName = "Mithril arrow";
			} else if (arrowcomboBox2.getSelectedItem().equals("Adamant Arrow")) {
				arrowID = adamantArrow;
				arrowName = "Adamant arrow";
			} else if (arrowcomboBox2.getSelectedItem().equals("Rune Arrow")) {
				arrowID = runeArrow;
				arrowName = "Rune arrow";
			} else if (arrowcomboBox2.getSelectedItem().equals("Bronze Bolt")) {
				arrowID = bronzeBolt;
				arrowName = "Bronze bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Bluerite Bolt")) {
				arrowID = blueriteBolt;
				arrowName = "Bluerite bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Bone Bolt")) {
				arrowID = boneBolt;
				arrowName = "Bone bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Iron Bolt")) {
				arrowID = ironBolt;
				arrowName = "Iron bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Steel Bolt")) {
				arrowID = steelBolt;
				arrowName = "Steel bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Black Bolt")) {
				arrowID = blackBolt;
				arrowName = "Bronze bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Mithril Bolt")) {
				arrowID = mithrilBolt;
				arrowName = "Mithril bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Adamant Bolt")) {
				arrowID = adamantBolt;
				arrowName = "Adamant bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Rune Bolt")) {
				arrowID = runeBolt;
				arrowName = "Rune bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Broad Bolt")) {
				arrowID = broadBolt;
				arrowName = "Broad bolts";
			} else if (arrowcomboBox2.getSelectedItem().equals("Bronze Knife")) {
				arrowID = bronzeKnife;
				arrowName = "Bronze knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Iron Knife")) {
				arrowID = ironKnife;
				arrowName = "Iron knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Steel Knife")) {
				arrowID = steelKnife;
				arrowName = "Steel knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Black Knife")) {
				arrowID = blackKnife;
				arrowName = "Black knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Mithril Knife")) {
				arrowID = mithrilKnife;
				arrowName = "Mithril knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Adamant Knife")) {
				arrowID = adamantKnife;
				arrowName = "Adamant knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Rune Knife")) {
				arrowID = runeKnife;
				arrowName = "Rune knife";
			} else if (arrowcomboBox2.getSelectedItem().equals("Bronze Dart")) {
				arrowID = bronzeDart;
				arrowName = "Bronze dart";
			} else if (arrowcomboBox2.getSelectedItem().equals("Iron Dart")) {
				arrowID = ironDart;
				arrowName = "Iron dart";
			} else if (arrowcomboBox2.getSelectedItem().equals("Steel Dart")) {
				arrowID = steelDart;
				arrowName = "Steel dart";
			} else if (arrowcomboBox2.getSelectedItem().equals("Black Dart")) {
				arrowID = blackDart;
				arrowName = "Black dart";
			} else if (arrowcomboBox2.getSelectedItem().equals("Mithril Dart")) {
				arrowID = mithrilDart;
				arrowName = "Mithril dart";
			} else if (arrowcomboBox2.getSelectedItem().equals("Adamant Dart")) {
				arrowID = adamantDart;
				arrowName = "Adamant dart";
			} else if (arrowcomboBox2.getSelectedItem().equals("Rune Dart")) {
				arrowID = runeDart;
				arrowName = "Rune dart";
			}
